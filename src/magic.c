...
int mag_savingthrow(struct char_data * ch, int type, int modifier)
{
  /* NPCs use knight tables according to some book */
  int class_sav = CLASS_KNIGHT;
  int save;

  if (!IS_NPC(ch))
    class_sav = GET_CLASS(ch);

  save = saving_throws(class_sav, type, GET_LEVEL(ch));/*REALLY FIX ME BRIAN*/
		      ...
		      int spellnum, int savetype)
{
  ...
  case SPELL_FLY:
    ...
    accum_duration = TRUE;
    to_vict = "You rise off the ground.";
	to_room = "$n beguns to hover inches above the ground.";
    break;
  }
		      ...
		      int spellnum, int savetype)
{
  ...
  for (i = 0; i < num; i++) {
    ...
    if (spellnum == SPELL_ANIMATE_DEAD){
	  mob->points.max_hit = number(10, GET_MAX_HIT(ch));
	  mob->points.hit = mob->points.max_hit;
	  mob->points.max_move = number(10, GET_MAX_MOVE(ch));
	  mob->points.move = mob->points.max_move;
	  mob->points.max_mana = number(10, GET_MAX_MANA(ch));
    ...
    if (spellnum == SPELL_CLONE) {	/* Don't mess up the proto with strcpy. */
	  ...
	  sprintf(buf, "A boring old clone of %s stands here.\r\n", GET_NAME(ch));
      mob->player.long_descr = str_dup(buf);
      mob->points.hit = (GET_MAX_HIT(ch)/2);
	  mob->points.max_hit = (GET_MAX_HIT(ch)/2);
	  mob->points.mana = (GET_MAX_MANA(ch)/2);
	  mob->points.max_mana = (GET_MAX_MANA(ch)/2);
    }
