...
extern room_rnum r_jailed_start_room;
...
int home_kingdom(struct char_data *d);
...
cpp_extern const struct command_info cmd_info[] = {
  ...
  { "call"	   , POS_STANDING, do_call     , 0, 0 },
  { "cast"     , POS_SITTING , do_cast     , 1, 0 },
  { "cackle"   , POS_RESTING , do_action   , 0, 0 },
  { "challenge", POS_RESTING , do_challenge, 0, 0 },
  ...
  { "decline"  , POS_RESTING , do_decline  , 0, 0 },
  ...
  { "force"    , POS_SLEEPING, do_force    , LVL_JUDGE, 0 },
  ...
  { "gaccept"  , POS_DEAD    , do_gaccept  , 0, 0 },
  { "gadvance" , POS_DEAD    , do_gadvance , 0, 0 },
  { "gamble"   , POS_STANDING, do_not_here , 0, 0 },
  { "gdemote"  , POS_DEAD    , do_gdemote  , 0, 0 },
  { "gecho"    , POS_DEAD    , do_gecho    , LVL_GOD, 0 },
  { "gexpel"   , POS_DEAD    , do_gexpel   , 0, 0 },
  ...
  { "goto"     , POS_SLEEPING, do_goto     , LVL_HELPER, 0 },
  ...
  { "heal"	   , POS_SITTING , do_not_here , 0, 0 },
  ...
  { "jail"     , POS_DEAD    , do_wizutil  , LVL_JUDGE, SCMD_JAIL },
  ...
  { "mute"     , POS_DEAD    , do_wizutil  , LVL_JUDGE, SCMD_SQUELCH },
  ...
  { "notitle"  , POS_DEAD    , do_wizutil  , LVL_JUDGE, SCMD_NOTITLE },
  ...
  { "omni"     , POS_DEAD    , do_omni     , LVL_HELPER, 0},
  ...
  { "pardon"   , POS_DEAD    , do_wizutil  , LVL_JUDGE, SCMD_PARDON },
  { "parole"   , POS_DEAD    , do_wizutil  , LVL_JUDGE, SCMD_PAROLE },
  { "peer"     , POS_RESTING , do_action   , 0, 0 },
  { "peace"    , POS_RESTING , do_peace    , LVL_HELPER, 0},
  ...
  { "population",POS_DEAD	 , do_population,0, 0},
  ...
  { "restore"  , POS_DEAD    , do_restore  , LVL_QUESTOR, 0 },
  ...
  { "roomflags", POS_DEAD    , do_gen_tog  , LVL_HELPER, SCMD_ROOMFLAGS },
  { "rp		  ", POS_RESTING , do_echo	   , 0, SCMD_RP },
  { "ruffle"   , POS_STANDING, do_action   , 0, 0 },

  { "say"      , POS_RESTING , do_say      , 0, 0 },
  { "'"        , POS_RESTING , do_say      , 0, 0 },
  { "save"     , POS_SLEEPING, do_save     , 0, 0 },
  { "sacrifice", POS_RESTING , do_sac	   , 0, 0 },
  ...
  { "send"     , POS_SLEEPING, do_send     , LVL_HELPER, 0 },
  { "set"      , POS_DEAD    , do_set      , LVL_QUESTOR, 0 },
  ...
  { "snoop"    , POS_DEAD    , do_snoop    , LVL_JUDGE, 0 },
  ...
  { "switch"   , POS_DEAD    , do_switch   , LVL_QUESTOR, 0 },
  ...
  { "train"    , POS_RESTING , do_train	   , 0, 0 },
  { "transfer" , POS_SLEEPING, do_trans    , LVL_JUDGE, 0 },
  { "treasury" , POS_DEAD    , do_treasure , 0, 0},
  ...
  { "unaffect" , POS_DEAD    , do_wizutil  , LVL_QUESTOR, SCMD_UNAFFECT },
  { "update"   , POS_DEAD    , do_update   , LVL_IMPL, 0 },
  ...
  { "world"    , POS_DEAD    , do_world    , LVL_IMPL, 0 },  
...
void command_interpreter(struct char_data *ch, char *argument)
{
  ...
  else if (GET_POS(ch) != POS_FIGHTING && GET_CHALLENGE(ch) == 1 && cmd_info[cmd].command != "ndecline" && cmd_info[cmd].command != "naccept")
	/*  if(cmd_info[cmd].command != "naccept"){
		  if(cmd_info[cmd].command != "ndecline"){*/
	  send_to_char("You must NACCEPT or NDECLINE.\r\n",ch);
		 /* }
...
void nanny(struct descriptor_data *d, char *arg)
{
  ...
  case CON_QRACE:
    ...
    if (load_result == RACE_OTHER) {
      SEND_TO_Q("\r\nThat's not a race.\r\nRace: ", d);
      return;
    } else
      GET_RACE(d->character) = load_result;
	  GET_DRACE(d->character) = load_result;
...
/*    SEND_TO_Q(class_menu, d);
  ...
  case CON_QCLASS:
    ...
    if (load_result == CLASS_UNDEFINED) {
	 ...
	 SEND_TO_Q(kingdom_menu, d);
	 SEND_TO_Q("\r\nKingdom: ",d);
	 STATE(d) = CON_QKINGDOM;
	 break;

  case CON_QKINGDOM:
	  load_result = parse_kingdom(*arg);
	  if(load_result == KING_NONE) {
		  SEND_TO_Q("/r/nThat's not a kingdom.\r\nKingdom: ", d);
		  return;
	  } else
		GET_KINGDOM(d->character) = load_result;
		kings[GET_KINGDOM(d->character)].population++;
	    GET_HOME(d->character) = home_kingdom(d->character);
  ...
  case CON_MENU:		/* get selection from main menu  */
	...
	load_room = real_room(load_room);
      ...
      if (load_room == NOWHERE) {
		...
		if(GET_HOME(d->character) != 0 && real_room(GET_HOME(d->character)) < 0)
