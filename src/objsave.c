...
/* local functions */
void Crash_extract_norents_eq(struct char_data *ch);
...
void auto_equip(struct char_data *ch, struct obj_data *obj, int location)
{
  ...
  if (location > 0) {	/* Was wearing it. */
	...
	break;
      if (IS_KNIGHT(ch) && CAN_WEAR(obj, ITEM_WEAR_WIELD) && GET_OBJ_TYPE(obj) == ITEM_WEAPON)
	break;
      location = LOC_INVENTORY;
      break;
    default:
  ...
  if (CMD_IS("rent")) {
   /* if (!(cost = Crash_offer_rent(ch, recep, FALSE, mode)))
    ...
    save_char(ch, save_room);
	 ...
	 act("$n says, '25 gold a day for rent or 1000 for Cryo.'",FALSE,recep,0,ch,TO_VICT);
