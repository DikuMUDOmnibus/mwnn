...
"enroll","expel","promote","demote","setfees","withdraw","setapplev"};
...
for (j = 0; j <= top_of_p_table; j++){
  if((victim=CON_PLAYING/*((player_table +j)->name)*/)) {
...
if(!(*arg)) {
  sprintf(buf, "\r");
  for(i=0; i < num_of_clans; i++)
    sprintf(buf, "%s[%-3d]  %-20s Members: %3d  Power: %5d  Appfee: %d  App Level: %d\r\n",buf , i, clan[i].name,clan[i].members,clan[i].power,clan[i].app_fee,clan[i].app_level);
...
send_to_char(clan[i].description, ch);
*/
