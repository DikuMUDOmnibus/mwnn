...
int mag_manacost(struct char_data * ch, int spellnum)
{
  return MAX(SINFO.mana_max - (SINFO.mana_change *
		    (GET_GLEVEL(ch) - SINFO.min_level[(int) GET_CLASS(ch)])),
	     SINFO.mana_min);
}
...
if(!IS_NPC(ch))
  return (call_magic(ch, tch, tobj, spellnum, GET_GLEVEL(ch), CAST_SPELL));
else 
  return (call_magic(ch, tch, tobj, spellnum, number(1,10), CAST_SPELL));
}
...
ACMD(do_cast)
{
  ...
  if (GET_GLEVEL(ch) < SINFO.min_level[(int) GET_CLASS(ch)] && GET_LEVEL(ch) < LVL_GOD) {
...
void mag_assign_spells(void)
{
  ...
  spello(SPELL_RESURRECT, "resurrection", 250, 150, 10, POS_STANDING,
  ...
  spello(SPELL_FLY, "fly", 20, 8, 1, POS_FIGHTING, TAR_CHAR_ROOM, FALSE, MAG_AFFECTS);
  ...
  skillo(SKILL_FOURTH, "fourth attack");
  skillo(SKILL_THROW,  "throw");
  skillo(SKILL_DAGGER, "dagger");
  skillo(SKILL_AXE, "axes");
  skillo(SKILL_BLUDGEON, "bludgeon");
  skillo(SKILL_SWORD, "swords");
  skillo(SKILL_POLEARM, "polearms");
  skillo(SKILL_WHIP, "whips");
  skillo(SKILL_EXOTIC, "exotics");
  skillo(SKILL_FLY, "fly");
  skillo(SKILL_BEARHUG, "bearhug");
  skillo(SKILL_BOW, "bow");
  skillo(SKILL_SLING, "sling");
  skillo(SKILL_CROSSBOW, "crossbow");
  skillo(SKILL_BLOCK, "block");
  skillo(SKILL_CALL_WILD, "call of the wild");
  skillo(SKILL_DODGE, "dodge");
  skillo(SKILL_DUAL_WIELD, "dual wield");
  skillo(SKILL_PEEK, "peek");
  skillo(SKILL_PARRY, "parry");
  skillo(SKILL_MOUNT, "mount");
  skillo(SKILL_RIDING, "riding");
  skillo(SKILL_TAME, "taming");
  skillo(SKILL_CIRCLE, "circle");
  skillo(SKILL_HUNT, "hunt");
  skillo(SKILL_CHARGE, "charge");
  skillo(SKILL_SPY, "spy");
  skillo(SKILL_BETRAY, "betray");
  skillo(SKILL_MEDITATE, "meditate");
  skillo(SKILL_BEAST, "beastform");
