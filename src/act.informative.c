...
extern char *ntitle_female[];
extern char *ntitle_male[];
...
void look_at_char(struct char_data * i, struct char_data * ch)
{
  ...
  if (ch != i && ((IS_THIEF(ch) && GET_SKILL(ch, SKILL_PEEK) > 0) || GET_LEVEL(ch) >= LVL_IMMORT)) {
    found = FALSE;
    act("\r\nYou attempt to peek at $s inventory:", FALSE, i, 0, ch, TO_VICT);
    for (tmp_obj = i->carrying; tmp_obj; tmp_obj = tmp_obj->next_content) {
      if (CAN_SEE_OBJ(ch, tmp_obj) && (number(1, 101) < GET_SKILL(ch, SKILL_PEEK))) {
...
void look_at_room(struct char_data * ch, int ignore_brief)
{
  ...
  if (IS_DARK(ch->in_room) && !CAN_SEE_IN_DARK(ch) && GET_RACE(ch) != RACE_ELF) {
...
ACMD(do_look)
{
  ...
  if (GET_POS(ch) < POS_SLEEPING)
    ...
    send_to_char("You can't see a damned thing, you're blind!\r\n", ch);
  else if (IS_DARK(ch->in_room) && !CAN_SEE_IN_DARK(ch) && GET_RACE(ch) != RACE_ELF) {
    send_to_char("It is pitch black...\r\n", ch);
...
ACMD(do_score)
{
  ...
  if (IS_NPC(ch))
    return;
sprintf (buf, "You are %s, the %s ",GET_NAME(ch), RACE_NAME(ch));
if (GET_CLASS(ch) == 0)
	sprintf(buf + strlen(buf), " Unguilded.\r\n");
else
	sprintf(buf + strlen(buf), "of the %s Guild (Guild Level: %d).\r\n", CLASS_NAME(ch), GET_GLEVEL(ch));
if ((clan_num=find_clan_by_id(GET_CLAN(ch)))>=0&&clan_num<num_of_clans) {
	if(GET_CLAN_RANK(ch)>0)
		sprintf(buf + strlen(buf), "You are %s of the %s Clan", clan[clan_num].rank_name[GET_CLAN_RANK(ch) -1],
                clan[clan_num].name);
	else
        sprintf(buf + strlen(buf), "You are applying to the %s Clan", clan[clan_num].name);
       }

  sprintf(buf + strlen(buf), "You are %d years old.", GET_AGE(ch));
  ...
  sprintf(buf +strlen(buf), "You have %d spendable Exp.\r\n", GET_S_EXP(ch));
  ...
  if (GET_SHARES(ch) > 0)
    sprintf(buf + strlen(buf), "You have %d shares worth a total of %d gold\r\n",
	    GET_SHARES(ch), GET_SHARES(ch) * share_value);
  ...
  sprintf(buf + strlen(buf), "You have %d RP points and %d Quest Points.\r\n",
	  GET_RPP(ch), GET_QP(ch));

  if(GET_NOBLE(ch) > 0){
	  switch(GET_SEX(ch)){
	  case SEX_MALE:
	  sprintf(buf + strlen(buf), "You are a noble of %s rank.\r\n", MROYAL_NAME(ch));
	  break;
	  case SEX_FEMALE:
	  sprintf(buf + strlen(buf), "You are a noble of %s rank.\r\n", FROYAL_NAME(ch));
	  break;
  ...
  if(GET_LEVEL(ch) == 1)
	  sprintf(buf + strlen(buf), "You are a Mortal.\r\n");
  else
	  sprintf(buf + strlen(buf), "You are a Immortal.\r\n");
  ...
  if (AFF_FLAGGED(ch, AFF_FLY))
    strcat(buf, "You hover in the air.\r\n");
...
ACMD(do_time)
{
  ...
  sprintf(buf, "It is the %d level, on ", time_info.hours);

  /* 35 days in a month */
  weekday = ((60 * time_info.month) + time_info.day + 1) % 10;
  ...
  sprintf(buf, "The %d%s Rising of the %s, Turn %d.\r\n",
...
ACMD(do_who)
{
  ...
  int who_room = 0, showrace = 0, clan_num = 0;
 ...
 if (*arg == '-') {
  ...
  for (d = descriptor_list; d; d = d->next) {
    ...
    if (short_list) {
	  ...
	  if(GET_LEVEL(tch) < LVL_IMMORT)
		  sprintf(buf, "[PlC]");
		  		  ...
		  		  sprintf(buf, "[Imm]");
	  else if (GET_LEVEL(tch) == LVL_HELPER)
		  		  sprintf(buf, "[Hlp]");
	  else if (GET_LEVEL(tch) == LVL_QUESTOR)
		  		  sprintf(buf, "[Qst]");
	  else if (GET_LEVEL(tch) == LVL_BUILDER)
		  		  sprintf(buf, "[Bld]");
	  else if (GET_LEVEL(tch) == LVL_JUDGE)
		  		  sprintf(buf, "[Jud]");
	  ...
	  if(GET_NOBLE(ch) > 0){
		  switch(GET_SEX(ch)){
		  case SEX_MALE:
			  sprintf(buf + strlen(buf), " %s ", MROYAL_NAME(tch));
			  break;
		  case SEX_FEMALE:
			  sprintf(buf + strlen(buf), " %s ", FROYAL_NAME(tch));
			  break;
			}
	  }
	...
	strcat(buf, " (KILLER)");
	  if (PLR_FLAGGED(tch, PLR_CRIMINAL))
    strcat(buf, " (JAILED)");
...
ACMD(do_users)
{
  const char *format = "%3d %-11s %-12s %-14s %-2s %-8s ";
  ...
  strcpy(line,
	 "Num Class       Name         State          Id Login@   Site\r\n");
  strcat(line,
	 "--- ----------- ------------ -------------- -- -------- ------------------------\r\n");
...
ACMD(do_consider)
{
  ...
  if (diff <= -100)
    send_to_char("You are GREATLY healthly.\r\n",ch);
  else if (diff <= -50)
    send_to_char("You are MUCH healthier.\r\n",ch);
  else if (diff <= -20)
    send_to_char("You are healthier.\r\n",ch);
  else if (diff <= -10)
    send_to_char("You are slightly healthier.\r\n",ch);
  else if (diff == 0)
    send_to_char("You look about the same.\r\n",ch);
  else if (diff <= 10)
    send_to_char("You are slightly weaker.\r\n",ch);
  else if (diff <= 20)
    send_to_char("You are weaker.\r\n",ch);
  else if (diff <= 30)
    send_to_char("You would need armour.\r\n",ch);
  else if (diff <= 50)
    send_to_char("You are MUCH weaker.\r\n",ch);
  else if (diff <= 100)
    send_to_char("You are GREATLY weaker\r\n",ch);
  else if (diff <= 500)
	send_to_char("You are ABSURDLY weaker.\r\n",ch);
  else
    send_to_char("Did you kiss your mother good-bye?\r\n",ch);
}
...
ACMD(do_scan)
{
  ...
  if ((GET_MOVE(ch) < 0) && (GET_LEVEL(ch) < LVL_IMMORT)) {
