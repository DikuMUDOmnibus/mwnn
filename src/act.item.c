...
void wear_message(struct char_data * ch, struct obj_data * obj, int where)
{
  const char *wear_messages[][2] = {
    ...
    {"$n put $p on $s ears.",
    "You put $p on your ears."},

    {"$n slide $p on $s tail.",
    "You slide $p on your tail."},

    {"$n wrap $p on $s wings.",
    "You wrap $p on your wings."}
  };
...
int find_eq_pos(struct char_data * ch, struct obj_data * obj, char *arg)
{
  ...
  if (!arg || !*arg) {
    ...
    if ((where = search_block(arg, body, FALSE)) < 0) {
...
ACMD(do_wear)
{
  ...
  if (dotmode == FIND_ALL) {
	...
	if ((where = find_eq_pos(ch, obj, 0)) >= 0  && !(GET_OBJ_LEVEL(obj) > GET_LEVEL(ch)))
	  perform_wear(ch, obj, where);
	else
...
ACMD(do_wield)
{
  ...
  if (!*arg)
    ...
    if (!CAN_WEAR(obj, ITEM_WEAR_WIELD) || GET_OBJ_LEVEL(obj) > GET_LEVEL(ch))
      send_to_char("You can't wield that.\r\n", ch);
    else if (GET_OBJ_WEIGHT(obj) > str_app[STRENGTH_APPLY_INDEX(ch)].wield_w)
      send_to_char("It's too heavy for you to use.\r\n", ch);
	else
		if (GET_OBJ_VAL(obj, 3) == TYPE_PIERCE - TYPE_HIT && !GET_SKILL(ch, SKILL_DAGGER) ||
		GET_OBJ_VAL(obj, 3) == TYPE_SLASH - TYPE_HIT && !GET_SKILL(ch, SKILL_SWORD) || 
		GET_OBJ_VAL(obj, 3) == TYPE_BLUDGEON - TYPE_HIT && !GET_SKILL(ch, SKILL_BLUDGEON) ||
		GET_OBJ_VAL(obj, 3) == TYPE_THRASH - TYPE_HIT && !GET_SKILL(ch, SKILL_AXE) ||
		GET_OBJ_VAL(obj, 3) == TYPE_CLAW - TYPE_HIT && !GET_SKILL(ch, SKILL_POLEARM) || 
		GET_OBJ_VAL(obj, 3) == TYPE_WHIP - TYPE_HIT && !GET_SKILL(ch, SKILL_WHIP) ||
		GET_OBJ_VAL(obj, 3) == TYPE_BOW - TYPE_HIT && !GET_SKILL(ch, SKILL_BOW) ||
		GET_OBJ_VAL(obj, 3) == TYPE_CROSSBOW - TYPE_HIT && !GET_SKILL(ch, SKILL_CROSSBOW) ||
		GET_OBJ_VAL(obj, 3) == TYPE_SLING - TYPE_HIT && !GET_SKILL(ch, SKILL_SLING)) 
