...
int is_ok_char(struct char_data * keeper, struct char_data * ch, int shop_nr)
{
  ...
  if ((IS_HUMAN(ch) && NOTRADE_HUMAN(shop_nr)) ||
      ...
      (IS_DRACO(ch) && NOTRADE_DRACO(shop_nr)) ||
	  (IS_GIANT(ch) && NOTRADE_GIANT(shop_nr)) ||
	  (IS_WERE(ch) && NOTRADE_WERE(shop_nr)) ||
	  (IS_GNOME(ch) && NOTRADE_GNOME(shop_nr)) ||
	  (IS_DWARF(ch) && NOTRADE_DWARF(shop_nr)) ||
	  (IS_VAMPYRE(ch) && NOTRADE_VAMPYRE(shop_nr)) ||
	  (IS_TROLL(ch) && NOTRADE_TROLL(shop_nr)) ||
	  (IS_ARIEL(ch) && NOTRADE_ARIEL(shop_nr)) ||
	  (IS_FAIRY(ch) && NOTRADE_FAIRY(shop_nr)) ||
	  (IS_MINOTAUR(ch) && NOTRADE_MINOTAUR(shop_nr)) ||
	  (IS_GHOUL(ch) && NOTRADE_GHOUL(shop_nr)) ||
	  (IS_GHOST(ch) && NOTRADE_GHOST(shop_nr)) ||
