...
extern sh_int r_mortal_start_room; /* This is necessary for parole */
...
ACMD(do_echo)
{
  ...
  if (!*argument)
    ...
    if (subcmd == SCMD_EMOTE)
      sprintf(buf, "<OOC>$n %s", argument);
	else if (subcmd == SCMD_RP){
		sprintf(buf, "$n %s", argument);
		if(strlen(argument) <= 20)
			GET_RPP(ch) += 5;
		else if(strlen(argument) <= 40)
			GET_RPP(ch) += 15;
...
void do_stat_character(struct char_data * ch, struct char_data * k)
{
  ...
  if (IS_NPC(k)) {	/* Use GET_CLASS() macro? */
	...
	sprintf(buf2+strlen(buf2), ", Guild Level: %d", GET_GLEVEL(k));
  }
  strcat(buf, buf2);
  ...
  if (!IS_NPC(k)) {
    ...
    sprintf(buf, "Kingdom: [%d], Hometown: [%d], Speaks: [%d/%d/%d], (STL[%d]/per[%d]/NSTL[%d])\r\n",
	k->player.kingdom, k->player.hometown, GET_TALK(k, 0), GET_TALK(k, 1), GET_TALK(k, 2),
	    GET_PRACTICES(k), int_app[GET_INT(k)].learn,
	    wis_app[GET_WIS(k)].bonus);
	sprintf(buf+strlen(buf), "RPP: [%7d], QP: [%4d]",GET_RPP(k),GET_QP(k));
  ...
  sprintf(buf, "Coins: [%9d], Bank: [%9d] (Total: %d), Shares: [%9d]\r\n",
  ...
  if (!IS_NPC(k)) {
    sprintf(buf, "Hunger: %d, Thirst: %d, Drunk: %d, Noble Rank: %d\r\n",
	  GET_COND(k, FULL), GET_COND(k, THIRST), GET_COND(k, DRUNK), GET_NOBLE(k));
    send_to_char(buf, ch);
  }
...
ACMD(do_advance)
{
  ...
  if (GET_LEVEL(ch) <= GET_LEVEL(victim) && !strcmp(GET_NAME(ch), "Curoehana")) {
...
{
  struct char_data *vict;
 // int i; Commented so Imms only get what they prac.. :)
...
ACMD(do_invis)
{
  ...
  if(GET_LEVEL(ch) == LVL_HELPER){
	  send_to_char("Sorry, but someone might need you.\r\n", ch);
	  return;
  }
  ...
  if (!*arg)
    ...
    case SCMD_JAIL:
     ...
     char_from_room(vict);
      ...
      act("The wardens come in and haul $n's butt to jail!", FALSE, vict, 0, 0, TO_ROOM);
      sprintf(buf, "(GC) %s jailed by %s.", GET_NAME(vict), GET_NAME(ch));
      mudlog(buf, BRF, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE);
      break;
   case SCMD_PAROLE:
      if (!PLR_FLAGGED(vict, PLR_CRIMINAL)) {
      ...
      if (GET_JAIL_LEV(vict) > GET_LEVEL(ch)) {
       sprintf(buf, "Sorry, a level %d Imm jailed %s... you can't parole %s.\r\n",
           GET_JAIL_LEV(vict), GET_NAME(vict), HMHR(vict));
        send_to_char(buf, ch);
        return;
      }
      sprintf(buf, "(GC) %s paroled by %s.", GET_NAME(vict), GET_NAME(ch));
     mudlog(buf, BRF, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE);
      REMOVE_BIT(PLR_FLAGS(vict), PLR_CRIMINAL);
      send_to_char("The board decides to give you another chance!\r\nYou've been Paroled.\r\n", vict);
      char_from_room(vict);
      char_to_room(vict, r_mortal_start_room);
      act("The gates of the jail open and $n reemerges!", FALSE, vict, 0, 0, TO_ROOM);
      break;
    default:
...
ACMD(do_show)
{
 ...
 sprintf(buf, "Player: %-12s (%s) [%2d %s %s]\r\n", vbuf.name,
    ...
    sprintf(buf + strlen(buf),
	 "Gold: %-8d  Bal: %-8d  Shr: %-8d   Align: %-5d\r\nExp: %-8d  S_Exp:%-8d  Lessons: %-3d\r\n",
	    vbuf.points.gold, vbuf.points.bank_gold, vbuf.points.bank_shares,
	    vbuf.char_specials_saved.alignment, vbuf.points.total_exp, vbuf.points.spend_exp,
...
/* The set options available */
  struct set_struct {
   ...
   { "rage",		LVL_IMPL,	PC,		NUMBER},
   { "trains",		LVL_QUESTOR, PC,	NUMBER},
   { "rank",		LVL_IMPL,	PC,		NUMBER}, /* 55 */
   { "shares",      LVL_GRGOD,  PC,     NUMBER },
   { "glevel",		LVL_GRGOD,	PC,		NUMBER},
   { "hometown",	LVL_GOD,    PC,		NUMBER},
   { "kingdom",		LVL_GOD,	PC,		NUMBER},
   { "\n", 0, BOTH, MISC }
  };
		...
		char *val_arg)
{
  ...
  case 20:
    ...
    break;
  case 21:
    vict->points.total_exp = RANGE(0, 50000000);
	vict->points.spend_exp = vict->points.total_exp;
    break;
  case 22:
  ...
  case 38:
    ...
    break;
  case 39:
    GET_CLASS(vict) = RANGE(0 , NUM_CLASSES);
    break;
  case 40:
  ...
  case 52:
    if ((i = parse_race(*val_arg)) == RACE_OTHER) {
  ...
  case 53:
    GET_RAGE(vict) = GET_RAGE(vict) + value;
    break;

  case 54:
	GET_TRAINING(vict) = RANGE(0, 26);
	break;

  case 55:
	  GET_NOBLE(vict) = RANGE(0, 9);
	  break;

  case 56:
	 GET_SHARES(vict) = RANGE(0, 1000000);
     break;

  case 57:
	  GET_GLEVEL(vict) = RANGE(0, 30);
	  break;
...
ACMD(do_world)
 {
   ...
   else if (strcmp(field, "pt") == 0)
   {
     if (pt_allowed == 1)
     {
       pt_allowed = 0;
       sprintf(buf, "[SYS: %s disallows player thieving]\r\n", GET_NAME(ch));
       send_to_imms(buf);
     }
     else
     {
       pt_allowed = 1;
       sprintf(buf, "[SYS: %s allows player thieving]\r\n", GET_NAME(ch));
       send_to_imms(buf);
     }
   }
   else
   {
	 ...
	 if (pt_allowed == 1) send_to_char("Pthieving allowed,\r\n", ch);
     if (pt_allowed == 0) send_to_char("Pthieving not allowed,\r\n", ch);
...
ACMD(do_omni) {
  ...
  send_to_char("Class  Room  Name                "
               "Health  Position\r\n", ch);
  for (tch = character_list; tch; tch = tch->next)
    if (!IS_NPC(tch) && GET_LEVEL(tch) < LVL_IMMORT) {
      sprintf(tmp, "%2s   %5d  %-20s %3d%%  %s\r\n",
              CLASS_ABBR(tch), world[IN_ROOM(tch)].number,
...
//Lvl  Class  Room  Name                Health  Position
//01    NC   12345  Tresni               100%   Standing
