...
ACMD(do_hunt)
{
  ...
  if (FIGHTING(ch)){
	  send_to_char("One can not hunt and fight  at the same time.", ch);
    return;
  }

  /* make sure the char still exists */
  if (!(tmp = get_char_vis(ch, arg, FIND_CHAR_WORLD))) {
    send_to_char("No one is around by that name.\r\n", ch);
    return;
  }

  if(!IS_NPC(tmp)){
	  send_to_char("You can not hunt another character.",ch);
	  return;
  }

  while(ch->in_room != tmp->in_room && !FIGHTING(ch) && (GET_SKILL(ch, SKILL_HUNT) >number(1,101) )&& (tmp = get_char_vis(ch, arg, FIND_CHAR_WORLD))){
	if ((dir = find_first_step(ch->in_room, tmp->in_room)) < 0) {
		do_say(ch, "Damn!  I lost my prey!", 0, 0);
		return;
	} else {
		if((SECT(ch->in_room) == SECT_INSIDE ) || (SECT(EXIT(ch, dir)->to_room) == SECT_INSIDE)) {
			send_to_char("You may not hunt indoors.\r\n", ch);
			return;
			}
		if((SECT(ch->in_room) == SECT_CITY ) || (SECT(EXIT(ch, dir)->to_room) == SECT_CITY)) {
			send_to_char("You may not hunt through cities.\r\n", ch);
			return;
			}
		perform_move(ch, dir, 1);
	}
