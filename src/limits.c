...
int mana_gain(struct char_data * ch)
{
  ...
  if (IS_NPC(ch)) {
    ...
    if (IS_MAGIC_USER(ch) && POS_MEDITATE)
      gain *= 2;
...
void gain_exp(struct char_data * ch, int gain)
{
  ...
  int num_levels = 0;
//  char buf[128];  took out for levelless system
...
void point_update(void)
{
  ...
  for (i = character_list; i; i = next_char) {
	...
	if(GET_RACE(i) == RACE_VAMPYRE && (time_info.hours < 45 || time_info.hours > 15)  && SECT(i->in_room) != SECT_INSIDE)
