...
if(GET_AVATAR(ch) == 0){
	send_to_char("You must be a noble to do that.\r\n", ch);
	return;
}
...
else if(GET_AVATAR(vict) == 0){
	send_to_char("Your victim must be an avatar.\r\n", ch);
	return;
}
...
ACMD(do_avatar){
	if(GET_AVATAR(ch) == 0){
		send_to_char("You will now be an avatar at level 30.\r\n", ch);
		GET_AVATAR(ch) = 1;
		return;
	}

	else if(GET_AVATAR(ch) == 1){
		if(GET_CHALLENGE(ch) != 0){
			send_to_char("You must not be challenged to end you Avatar status.\r\n", ch);
			return;
		}
		else{
		send_to_char("You will no longer be noble at level 30.\r\n(You can only turn this option off till level 20!)\r\n", ch);
	GET_AVATAR(ch) = 0;
		}
	}
}
