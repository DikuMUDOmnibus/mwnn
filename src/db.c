...
room_rnum r_jailed_start_room;     /* rnum of jailed start room     */
...
int race_weight(struct char_data *ch);
int race_height(struct char_data *ch);
...
extern room_vnum jailed_start_room;
...
void reset_time(void)
{
  ...
  else if (time_info.hours <= 48)
    weather_info.sunlight = SUN_LIGHT;
  else if (time_info.hours == 49)
...
void check_start_rooms(void)
{
  ...
  if ((r_jailed_start_room = real_room(jailed_start_room)) < 0) {
    ...
    r_jailed_start_room = r_jailed_start_room;
   }
}
...
void store_to_char(struct char_file_u * st, struct char_data * ch)
{
  ...
  GET_RACE(ch) = st->race;
  GET_BEAST(ch) = st->beast;
  GET_LEVEL(ch) = st->level;
  GET_NOBLE(ch) = st->rank;
  GET_PKILLS(ch) = st->pkills;
  GET_PDEATHS(ch) = st->pdeath;
  GET_MKILLS(ch) = st->mkills;
  GET_MDEATHS(ch) = st->mdeath;
  GET_RAGE(ch) = st->rage;
...
void char_to_store(struct char_data * ch, struct char_file_u * st)
{
  ...
  st->race = GET_RACE(ch);
  st->beast = GET_BEAST(ch);
  st->level = GET_LEVEL(ch);
  st->rage = GET_RAGE(ch);
  st->rank = GET_NOBLE(ch);
  st->pkills = GET_PKILLS(ch);
  st->pdeath = GET_PDEATHS(ch);
  st->mdeath = GET_MDEATHS(ch);
  st->mkills = GET_MKILLS(ch);
...
int file_to_string_alloc(const char *name, char **buf)
{
  ...
  struct descriptor_data *using;

  for (using = descriptor_list; using; using = using->next)
    if (using->showstr_vector && *using->showstr_vector == *buf)
      return (-1);
...
void init_char(struct char_data * ch)
{
  ...
  ch->player.rage = 0;
  ch->player.rank = 0;
  ch->player.challenge = 0;
  ch->player.challenger = NULL;
  ch->player.pkills = 0;
  ch->player.pdeath = 0;
  ch->player.mdeath = 0;
  ch->player.mkills = 0;
 ...
 // if (ch->player.sex == SEX_MALE) {
    ch->player.weight = race_weight(ch);
    ch->player.height = race_height(ch);
 // } else {
 //   ch->player.weight = race_weight(ch);
 //   ch->player.height = race_height(ch);
 // }
...
int check_object_level(struct obj_data *obj, int val)
{
  ...
  if ((GET_OBJ_VAL(obj, val) < 0 || GET_OBJ_VAL(obj, val) > MAX_GLEVEL) && (error = TRUE))
...
int race_weight(struct char_data *ch){
	...
	case RACE_GNOME:
	case RACE_DRACO:
		return(number(150, 200));
	case RACE_TROLL:
	case RACE_MINOTAUR:
		return(number(200, 250));
	case RACE_GIANT:
		return(number(250, 350));
	case RACE_KENDER:
		return(number(80, 110));
	case RACE_FAIRY:
	case RACE_GHOST:
		return(number(3, 6));
	default:
		log("BRIAN YOU FORGOT A RACE WEIGHT!!");
...
int race_height(struct char_data *ch){
	...
	case RACE_MINOTAUR:
		return(number(72, 78));
	case RACE_GIANT:
		return(number(132, 144));
	case RACE_FAIRY:
		return(1);
	default:
		log("BRIAN YOU FORGOT A RACE HEIGHT!!");
		return(number(100, 200));
	}
}
