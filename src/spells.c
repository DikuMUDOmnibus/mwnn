...
ASPELL(spell_resurrect)
{
	...
	if (IS_NPC(victim) || GET_DEAD(ch) == FALSE || GET_DRACE(victim) == RACE_GHOST){
		send_to_char("Can not resurrect those people.\r\n",ch);
		return;
	}

	GET_RACE(victim) = GET_DRACE(victim);

	act("$n slowly appears from the air as $s body is resurrected.",TRUE, victim, 0, 0, TO_ROOM);
